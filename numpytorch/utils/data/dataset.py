...
class DataLoader:
    def __init__(self, X: np.array, y: np.array, batch_size: int = 32, shuffle: bool = True):
        ...
        assert len(
            y.shape) > 1, "y data must have atleast 2 dims. shape: (n, input_size)"
...
class TestTrainDataLoader:
    ...
    def load_mnist(self, image_path, label_path):
        ...
        with gzip.open(images_path, 'rb') as imgpath:
            images = np.frombuffer(
                imgpath.read(), dtype=np.float32, offset=16).reshape(len(labels), 784)
    ...
    def convert_to_mnist(self):
        ...
        subdirs = [d for d in os.listdir(self.image_root_folder) if os.path.isdir(
            os.path.join(self.image_root_folder, d))]
        ...
        print(
            f"Converted {len(images)} images from {len(subdirs)} folders to MNIST format and saved to {self.image_root_folder}")
            ...
            for filename in os.listdir(folder_path):
                if filename.endswith(".jpg"):
                    ...
                    txt_file_path = os.path.join(
                        folder_path, file_name_without_ext + ".txt")
    ...
    def split_data(self):
        ...
        print(
            f"Dataset split into training set ({len(train_images)} samples) and testing set ({len(test_images)} samples)")
